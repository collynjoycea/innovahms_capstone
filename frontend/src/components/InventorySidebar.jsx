...
const InventorySidebar = ({ isDarkMode }) => {
  const location = useLocation();
  const navigate = useNavigate();
  const [isOnline, setIsOnline] = useState(navigator.onLine);
  const [time, setTime] = useState(new Date());

  // Connection para sa Low Stock Alerts count
  ...
  const menuSections = [
    {
      title: "OVERVIEW",
      items: [{ name: "Dashboard", path: "/inventory/dashboard", icon: <LayoutDashboard /> }]
    },
    {
      title: "STOCK CONTROL",
      items: [
        { name: "Inventory List", path: "/inventory/stock", icon: <ClipboardList /> },
        { 
          name: "Stock In / Out", 
    ...
    },
    {
      title: "ANALYTICS",
      items: [
        { name: "Consumption Reports", path: "/inventory/reports", icon: <BarChart3 /> },
        { 
    ...
    },
    {
      title: "SYSTEM",
      items: [
        { name: "History Logs", path: "/inventory/history", icon: <History /> },
  ...
  return (
    <aside className={`w-[260px] ${borderColor} border-r h-screen sticky top-0 font-sans transition-all duration-300 ${bgColor}`}>
      <div className="h-full overflow-y-auto custom-sidebar-scroll flex flex-col text-left">
        ...
        {/* 1. BRAND LOGO & NETWORK STATUS (Eksaktong gaya ng HR Sidebar) */}
        <div className="px-7 py-6 flex items-center justify-between shrink-0">
          <Link to="/inventory/dashboard" className="transition-transform hover:scale-105 text-left">
        ...
        <div className="px-4 mb-6 shrink-0">
          <div className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${isDarkMode ? 'bg-[#2FA084]/5 border-[#2FA084]/10' : 'bg-white border-gray-200 shadow-sm'}`}>
            ...
            <div className="overflow-hidden">
              ...
              <p className="text-[8px] font-bold text-[#2FA084] uppercase tracking-widest mt-1.5 opacity-80">Inventory Manager</p>
            </div>
          </div>
        </div>
                  ...
                  >
                    ...
                    {item.badge && (
                      <span className={`text-[8px] font-black px-1.5 py-0.5 rounded-md text-center ${
                        item.badge === "AUTO" ? 'bg-emerald-600/20 text-emerald-500 border border-emerald-500/30' : 
        ...
        {/* 4. FOOTER: DIGITAL CLOCK & LOGOUT (Eksaktong gaya ng HR Sidebar) */}
